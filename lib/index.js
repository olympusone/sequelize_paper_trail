			...
			try {
				...
				if (!destroyOperation) {
					...
					if (changedKeys.length) {
						...
						query[options.versionAttributes.object] = object;
						query[options.versionAttributes.objectChanges] = delta;
					}
				}
				...
				// Save version
				return await version.save({ transaction: opt.transaction })
			} catch (err) {
	...
	return {
		defineModels(db) {
			...
			const attributes = {
				...
				[options.versionAttributes.object]: Sequelize.JSONB,
				[options.versionAttributes.objectChanges]: Sequelize.JSONB,
